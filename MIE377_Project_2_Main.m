...
for t = 1 : NoPeriods
    ...
    f_bar = geo_mean(1 + periodFactRet) - 1;      % m x 1 vector of factor expected returns
    ...
    [x{5}(:,t),CVaR(t,1),VaR(t,1)] = funList{5}(mu, Q, currentPrices);
    ...
    for i = 1 : NoMethods
        ...
        periodportfValue{i} = periodPrices * NoShares{i};
        ...
        ex_ante_sharpe(t,i)= mu'*x{i}(:,t)/sqrt(x{i}(:,t)'*Q*x{i}(:,t));
        
        % Calculate Ex Post Sharpe Ratio
        Per_Period_portfolio_return_i = periodportfValue{i}(2:end) ./ periodportfValue{i}(1:end - 1) - 1;
        Per_Period_portf_avg_return(t,i) = geo_mean(1 + Per_Period_portfolio_return_i) - 1;
        Per_Period_portf_variance(t,i) = var(Per_Period_portfolio_return_i);
        ex_post_sharpe(t,i)= Per_Period_portf_avg_return(t,i)/sqrt(Per_Period_portf_variance(t,i));
        %------------------------------------------------------------------
...
disp('Average Ex ante Sharpe Ratio:');
disp(mean(ex_ante_sharpe));
disp('Average Ex post Sharpe Ratio:');
disp(mean(ex_post_sharpe));
...
title('Portfolio Values of five models', 'FontSize', 14)
